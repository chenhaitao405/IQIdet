# IQIdet 集成速查

## 1. 前置条件

### 环境
```bash
conda activate weld-gpu
# Python: /home/cht/miniconda3/envs/weld-gpu/bin/python
pip install -r requirements.txt
```

### 模型文件
`models/` 目录下需存在：

| 文件 | 说明 |
|------|------|
| `guagerotation.pt` | YOLO-OBB ROI 检测权重 |
| `fclip67.pth.tar` | FClip 像质丝模型 (**Git LFS**) |
| `fclip_config.yaml` | FClip 模型配置 |
| `OCR_rec_inference_best_accuracy/` | PaddleOCR 识别模型 |
| `ocr_orientation_model.pth` | 文本方向矫正模型 |

```bash
git lfs install
git lfs pull --include="models/fclip67.pth.tar"
```

## 2. 功能速查

### 2.1 底片信息自动识别

- **入口**: `run_iqi_grade_infer.py`
- **最小命令**:
  ```bash
  python run_iqi_grade_infer.py \
    --image-dir IQIdata/ori/img \
    --output-json outputs/iqi_grade_results.json \
    --ocr-number-range 1-19
  ```
  （其余参数有默认值，指向 `models/` 下对应文件）
- **输出核心字段**: `ok`, `result_code`, `grade`, `iqi_type`, `wire_count`, `fields.weld_numbers`, `fields.film_numbers`
- **负责**: 整图 → 全流程 IQI 等级输出（OCR + ROI检测 + 丝数识别 + 等级计算）
- **不负责**: 前端交互、单区域实时处理
- **详文**: [docs/README_IQI_GRADE_INFERENCE_DELIVERY.md](docs/README_IQI_GRADE_INFERENCE_DELIVERY.md)

### 2.2 区域 OCR 识别

- **入口**: `region_ocr_api.py`
- **核心调用**:
  ```python
  from region_ocr_api import (
      init_region_ocr_api, recognize_region, close_region_ocr_api,
  )
  init_region_ocr_api()                # 服务启动时一次
  result = await recognize_region(req) # 每请求
  close_region_ocr_api()               # 服务退出时
  ```
- **输入**: 单个已裁剪文字区域的 base64 图像（支持 `data:image/png;base64,...`）
- **输出**: `status`, `text`, `normalized_text`, `score`, `orientation`
- **负责**: 图像增强 → 方向矫正 → 文字识别 → 文本标准化
- **不负责**: 文本检测、多区域切分、整图方向矫正

### 2.3 区域归一化信噪比

- **入口**: `region_SNR_api.py`
- **核心调用**:
  ```python
  from region_SNR_api import (
      init_region_snr_api, compute_region_snr, close_region_snr_api,
  )
  init_region_snr_api(sr_b_um=88.6)      # 服务启动时一次，设置 sr_b_um 默认值（单位：μm）
  result = await compute_region_snr(req)  # 每请求（可传入 sr_b_um 覆盖默认值）
  close_region_snr_api()                  # 服务退出时
  ```
- **输入**: 单个已裁剪区域的 base64 图像（`SNRRequest.image_base64`），可选传入 `sr_b_um`（`SNRRequest.sr_b_um` 或函数参数 `compute_region_snr(req, sr_b_um=...)`，**单位：μm**）
- **输入 `sr_b_um` 优先级**（由高到低）：
  1. 函数参数 `compute_region_snr(req, sr_b_um=...)`
  2. 请求体字段 `SNRRequest.sr_b_um`
  3. 初始化时的默认值 `init_region_snr_api(sr_b_um=...)`
- **输出**: `snr_m`, `snr_n`, `gray_mean`, `gray_std`, `result_code`, `sr_b_um`
- **负责**: 灰度统计 → 测量信噪比 → 归一化信噪比
- **不负责**: 文本检测、OCR、多区域切分
- **注意**: 区域面积需 ≥ 20×55 像素，否则返回 `4001`
- **公式**:
  - `snr_m = gray_mean / gray_std`（测量信噪比）
  - `snr_n = snr_m × 88.6 / sr_b_um`（归一化到 88.6 μm 标准分辨力）

### 2.4 双丝像质计分辨率分析

- **入口**: `double_wire_api.py`
- **核心调用**:
  ```python
  from double_wire_api import (
      init_double_wire_api, compute_double_wire, close_double_wire_api,
  )
  init_double_wire_api()                    # 服务启动时一次
  result = await compute_double_wire(req)   # 每请求
  close_double_wire_api()                   # 服务退出时
  ```
- **输入**: 剖面线 strip 条带图像 base64（调用方完成拉线和条带提取，图像为沿剖面线的子像素采样灰度图，支持 `data:image/png;base64,...`）
- **输出**: `profile`（灰度曲线）、`background`（背景拟合）、`peaks`/`valleys`（峰谷坐标）、`pairs`（每组丝的对比度与位置）、`film_type`（正/负片）、`first_unresolved_group`（首个不可分辨组号）
- **负责**: strip 列平均 → 背景拟合 → 峰谷检测 → 丝配对 → dip 计算 → 分辨率判定
- **不负责**: 剖面线绘制、strip 条带提取、前端渲染
- **详文**: [docs/contract/DOUBLE_WIRE_ANALYSIS_API.md](docs/contract/DOUBLE_WIRE_ANALYSIS_API.md)
- **集成验证**: [test_data/double_wire_api_test/](test_data/double_wire_api_test/) — 参考输入/输出，验证方法见目录内 README

## 3. 详细文档

- [交付字段与 JSON schema](docs/README_IQI_GRADE_INFERENCE_DELIVERY.md)
- [代码架构](ARCHITECTURE.md)
- [模型资产](docs/MODEL_ASSETS.md)
